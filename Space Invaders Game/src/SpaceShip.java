...
public class SpaceShip extends MovingObject {
     ...
     * 
     * @param window the GWindow this SpaceShip belongs to
     * @param center the center Point of this SpaceShip
     */
	...
	public void move() {
		...
		do {
            ...
            } else if (y + this.boundingBox.getHeight() / 2 >= this.window.getWindowHeight()) {
	...
	public void shoot(ArrayList<Alien> aliens) {
		...
		for (int i = 0; i < aliens.size(); i++)	{
			...
			if (this.center.x >= r.getX() && this.center.x <= r.getX() + r.getWidth())	{
				a.isShot();
			}
		}
